#!/usr/bin/env python3
"""
Test API with minimal data to identify which fields are causing issues
"""
...
async def test_minimal_api():
    ...
    try:
        # Test with only the core fields that we know work
        test_data = {
            "keyword": "test minimal api",
            "search_volume": 1000,
        ...
        for field_name, field_value in additional_fields:
            try:
                test_data_with_field = test_data.copy()
                test_data_with_field[field_name] = field_value
                test_data_with_field["keyword"] = f"test {field_name}"
                
                logger.info(f"Testing field: {field_name}")
                response = supabase.table("keyword_research_data").insert(test_data_with_field).execute()
                logger.info(f"✅ Field {field_name} works")
            except Exception as e:
                logger.error(f"❌ Field {field_name} failed: {e}")
