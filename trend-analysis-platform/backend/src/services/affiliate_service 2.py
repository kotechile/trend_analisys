...
class AffiliateService:
    ...
    async def _research_shareasale(self, topic: str, search_query: str) -> List[Dict[str, Any]]:
        """Research ShareASale network using real API"""
        try:
            from ..integrations.affiliate_networks import shareasale_api
            programs = await shareasale_api.search_programs(topic, limit=20)
            return programs
        except Exception as e:
    ...
    async def _research_impact(self, topic: str, search_query: str) -> List[Dict[str, Any]]:
        """Research Impact network using real API"""
        try:
            from ..integrations.affiliate_networks import impact_api
            programs = await impact_api.search_programs(topic, limit=20)
            return programs
        except Exception as e:
    ...
    async def _research_amazon(self, topic: str, search_query: str) -> List[Dict[str, Any]]:
        """Research Amazon Associates using real API"""
        try:
            from ..integrations.affiliate_networks import amazon_api
            programs = await amazon_api.search_programs(topic, limit=20)
            return programs
        except Exception as e:
    ...
    async def _research_cj(self, topic: str, search_query: str) -> List[Dict[str, Any]]:
        """Research Commission Junction using real API"""
        try:
            from ..integrations.affiliate_networks import cj_api
            programs = await cj_api.search_programs(topic, limit=20)
            return programs
        except Exception as e:
    ...
    async def _research_partnerize(self, topic: str, search_query: str) -> List[Dict[str, Any]]:
        """Research Partnerize network using real API"""
        try:
            from ..integrations.affiliate_networks import partnerize_api
            programs = await partnerize_api.search_programs(topic, limit=20)
            return programs
        except Exception as e:
    ...
    async def _research_mock_network(self, network_id: str, topic: str, search_query: str) -> List[Dict[str, Any]]:
        """Research other networks using real APIs when available"""
        try:
            # Try to use real API for remaining networks
            from ..integrations.affiliate_networks import get_network_api
            api = get_network_api(network_id)
            if api:
                programs = await api.search_programs(topic, limit=20)
                return programs
        except Exception as e:
            logger.error(f"API error for {network_id}: {e}")
