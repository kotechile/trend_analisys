...
class LLMConfigManager:
    ...
    def _load_configurations(self):
        ...
        # Kimi2 Configuration
        kimi_key = os.getenv("KIMI2_API_KEY")
        if kimi_key:
            self.configs[LLMProvider.KIMI2] = LLMConfig(
                provider=LLMProvider.KIMI2,
                api_key=kimi_key,
                model=os.getenv("KIMI2_MODEL", "moonshot-v1-8k"),
                max_tokens=int(os.getenv("KIMI2_MAX_TOKENS", "1000")),
                temperature=float(os.getenv("KIMI2_TEMPERATURE", "0.7")),
                base_url=os.getenv("KIMI2_BASE_URL", "https://api.moonshot.cn/v1")
            )
            logger.info("Kimi2 configuration loaded")
        ...
        if deepseek_key:
            ...
            logger.info("DeepSeek configuration loaded")
        ...
        if qwen_key:
            ...
            logger.info("Qwen configuration loaded")
