"""
Standalone FastAPI app for content generation (no database dependencies)
"""
...
app = FastAPI(title="TrendTap Content API", version="0.1.0")
...
@app.get("/")
async def root():
    return {"message": "TrendTap Content Generation API"}

@app.get("/health/")
async def health():
    return {"status": "healthy", "service": "TrendTap Content API"}
