...
class TestAffiliateResearchContract:
    ...
    def test_post_affiliate_research_contract(self):
        """Test POST /api/affiliate-research/search contract"""
        # Test the actual implemented endpoint
        payload = {
            "topic": "home coffee roasting",
            "search_term": "coffee roasting equipment affiliate programs"
        }

        response = self.client.post("/api/affiliate-research/search", json=payload, headers={"Host": "localhost"})

        # Contract requirements - endpoint returns 200, not 201
        assert response.status_code == 200
        data = response.json()
        
        # Required fields in response based on actual API
        ...
        assert "total_programs" in response_data
        
        # Verify programs array is not empty
        assert len(response_data["programs"]) > 0
        
        # Check first program structure
        first_program = response_data["programs"][0]
        assert "id" in first_program
        assert "name" in first_program
        assert "description" in first_program
        assert "commission" in first_program
