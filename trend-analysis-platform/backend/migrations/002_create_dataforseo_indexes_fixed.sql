-- Migration: Create DataForSEO indexes (Fixed Version)
-- Description: Creates additional indexes for performance optimization
-- Created: 2024-01-15
-- Author: DataForSEO Integration
-- Note: This is the fixed version without time-based partial indexes
...
-- Partial indexes for common filter conditions (without time-based predicates)
...
COMMENT ON INDEX idx_trend_analysis_data_average_interest IS 'Index on average interest for trend analysis queries';
COMMENT ON INDEX idx_trend_analysis_data_peak_interest IS 'Index on peak interest for trend analysis queries';
COMMENT ON INDEX idx_trend_analysis_data_location_time IS 'Composite index on location and time range for trend analysis';
COMMENT ON INDEX idx_keyword_research_data_cpc IS 'Index on cost per click for keyword research';
COMMENT ON INDEX idx_keyword_research_data_competition_value IS 'Index on competition value for keyword research';
COMMENT ON INDEX idx_keyword_research_data_trend_percentage IS 'Index on trend percentage for keyword research';
COMMENT ON INDEX idx_subtopic_suggestions_search_volume IS 'Index on search volume for subtopic suggestions';
COMMENT ON INDEX idx_subtopic_suggestions_commercial_intent IS 'Index on commercial intent for subtopic suggestions';
COMMENT ON INDEX idx_subtopic_suggestions_competition_level IS 'Index on competition level for subtopic suggestions';
