      ...
      switch (contentType) {
        case 'content':
          result = selectedPlatform === 'google-docs' 
            ? await exportToGoogleDocs({ content_id: itemId, template_id: selectedTemplate as number, custom_fields: customFields })
            : selectedPlatform === 'notion'
            ? await exportToNotion({ content_id: itemId, template_id: selectedTemplate as number, custom_fields: customFields })
            : await exportToWordPress({ content_id: itemId, template_id: selectedTemplate as number, custom_fields: customFields });
          break;
        case 'software':
                ...
                <Card>
                  <CardContent>
                    ...
                    {templates.find(t => t.id === selectedTemplate)?.fields && (
                      <Box mt={1}>
                        <Typography variant="caption" color="text.secondary">
                          Available fields: {Object.keys(templates.find(t => t.id === selectedTemplate)?.fields || {}).join(', ')}
                        </Typography>
                      </Box>
                    )}
                  </CardContent>
