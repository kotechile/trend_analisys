...
export const KeywordClusters: React.FC<KeywordClustersProps> = ({ keywordDataId }) => {
  ...
  const filteredClusters = clusters.filter((cluster: any) =>
    ...
    cluster.keywords.some((keyword: any) => 
                    ...
                    <TableRow key={cluster.id} hover>
                      ...
                      <TableCell>
                        <Box display="flex" flexWrap="wrap" gap={0.5} maxWidth={300}>
                          {cluster.keywords.slice(0, 3).map((keyword: any, index: number) => (
                            <Chip
                              key={index}
                              label={keyword}
                              size="small"
              ...
              <Grid item xs={12} sm={4}>
                <Paper sx={{ p: 2, textAlign: 'center' }}>
                  <Typography variant="h4" color="success.main">
                    {clusters.filter((c: any) => c.priority_score > 7).length}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    High Priority
                  </Typography>
              ...
              <Grid item xs={12} sm={4}>
                <Paper sx={{ p: 2, textAlign: 'center' }}>
                  <Typography variant="h4" color="warning.main">
                    {clusters.filter((c: any) => c.competition_level === 'low').length}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    Low Competition
                  </Typography>
